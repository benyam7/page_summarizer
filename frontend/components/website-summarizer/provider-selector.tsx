...
interface ProviderSelectorProps {
  ...
  onSelectProvider: (provider: "openai" | "ollama" | "anthropic" | "google" | "groq") => void
}
...
export const ProviderSelector = ({ selectedProvider, onSelectProvider }: ProviderSelectorProps) => {
  ...
  const selectedProviderInfo = providers.find((p) => p.id === selectedProvider) || providers[0]

  return (
    <div className="mb-4">
      <div className="text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2">Select LLM Provider</div>

      <div className="relative">
        {/* Selected Provider Card (Always visible) */}
        <div className="relative z-10 cursor-pointer" onClick={() => setExpanded(!expanded)}>
          <motion.div
            className={`flex items-center gap-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-700 ${expanded ? "rounded-b-none" : ""}`}
            animate={{
              backgroundColor: expanded ? "rgba(244, 244, 245, 0.8)" : "rgba(255, 255, 255, 0)",
              borderColor: expanded ? "rgba(228, 228, 231, 1)" : "rgba(228, 228, 231, 0.5)",
            }}
            transition={{ duration: 0.2 }}
          >
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${selectedProviderInfo.color}`}>
              {selectedProviderInfo.logo}
            </div>
            <div className="flex-1">
              <p className="font-medium text-zinc-900 dark:text-zinc-100">{selectedProviderInfo.name}</p>
              <p className="text-xs text-zinc-500">{selectedProviderInfo.description}</p>
            </div>
            <motion.div
            ...
            (provider) =>
              provider.id !== selectedProvider && (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-zinc-900 dark:text-zinc-100">{provider.name}</p>
